...
class Maze:
    ...
    def check(self):
        ...
        for x in directions.keys():
            ...
            if self.valid(temp) and self.maze[temp[0]][temp[1]] == conditions["frog"]:
                warnings.append(
                    frog_warnings[random.randint(0, len(frog_warnings) - 1)]
                )
            if self.valid(temp) and self.maze[temp[0]][temp[1]] == conditions["nebula"]:
                warnings.append(
                    nebula_warnings[random.randint(0, len(nebula_warnings) - 1)]
                )
    ...
    def valid(self, temp_position):
        return (
            temp_position[0] > -1
            and temp_position[0] < self.dimension
            and temp_position[1] > -1
            and temp_position[1] < self.dimension
        )
